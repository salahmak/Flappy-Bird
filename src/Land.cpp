...
Land::Land(GameDataRef data) : _data(data)
{
    ...
    sf::Sprite sprite2(this->_data->assets.GetTexture("land"));
   	...
   	sprite.setPosition(0, (SCREEN_HEIGHT - (sprite.getGlobalBounds().height / 2)));
   	sprite2.setPosition(sprite.getGlobalBounds().width , (SCREEN_HEIGHT - (sprite.getGlobalBounds().height / 2)));
    ...
    for(unsigned short int i = 0; i < _landSprites.size(); ++i)
        {
        	...
        	if(_landSprites[i].getPosition().x < -_landSprites[i].getGlobalBounds().width)
        	{
        		sf::Vector2f position(SCREEN_WIDTH, _landSprites[i].getPosition().y);
